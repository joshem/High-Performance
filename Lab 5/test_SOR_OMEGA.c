...
#define BASE  2
#define ITERS 1
#define DELTA 850        // Right now only set up to do one size
...
#define O_ITERS 8       // # of OMEGA values to be tested
#define PER_O_ITERS 1    // trials per OMEGA value
double OMEGA = 1.95;     // OMEGA base - first OMEGA tested
...
main(int argc, char *argv[])
{
  ...
  printf("\n MAXSIZE = %d", MAXSIZE);
  printf("\n Omega");
	for(i=1;i<PER_O_ITERS+1;i++)	printf(", Trial %d", i);
  ...
  for (i = 0; i < O_ITERS; i++) {
    ...
    for (j = 0; j < PER_O_ITERS; j++) {
      ...
      SOR(v0,iterations);
      acc += (double)(*iterations);  //adds all the iterations of this particular loop
      printf(", %d", *iterations);
    }
    convergence[i][0] = OMEGA;
    convergence[i][1] = acc/(double)(PER_O_ITERS); //takes the avg?
    OMEGA += OMEGA_INC;
  }
	printf("\nOmega,Average Iters");
  for (i = 0; i < O_ITERS; i++)
